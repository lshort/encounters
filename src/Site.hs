...
------------------------------------------------------------------------------
-- | 
getSplices :: [TableDef] -> [(DT.Text,Splice AppHandler)]
getSplices t = 
...
--   and uses this data as the initial value in an input element
--   also saves the data
sqlDropSave :: [TableDef] -> Splice AppHandler
sqlDropSave ts = do
    maybeSave ts
    sqlDropForm

sqlDropForm :: Splice AppHandler
sqlDropForm  = do
    node <- getParamNode
    req <- lift getRequest
    let column      = ColumnName $ decode node req "column"
...
sqlDrop :: Bool -> Splice AppHandler
sqlDrop none_opt = do
    node <- getParamNode
    req <- lift getRequest
    let column      = ColumnName $ decode node req "column"
...
-- | Uses tag attributes to run an SQL query where key = value
--   and uses this data as the initial value in an input element
sqlTextInput :: Splice AppHandler
sqlTextInput = do
    node <- getParamNode
    req <- lift getRequest
    let column = ColumnName $ decode node req "column"
...
uriHiddenInput :: Splice AppHandler
uriHiddenInput = do
    node <- getParamNode
    req <- lift getRequest
...
-- | Pulls node attributes from the splice and from the URI query data
--   and uses this data as the name, label, and initial value for an input element
uriTextInput :: Splice AppHandler
uriTextInput = do
    node <- getParamNode
    req <- lift getRequest
...
uriTextWithSave :: [TableDef] -> Splice AppHandler
...
maybeSave :: [TableDef] -> Splice AppHandler
maybeSave tables = do
    req <- lift getRequest
    node <- getParamNode
...
deleteFromTable req t key@(UriKeyName k) = do
    conn <- connectSqlite3 "encounter_db"
